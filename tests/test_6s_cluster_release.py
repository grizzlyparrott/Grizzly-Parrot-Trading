...
class SixSClusterReleaseTests(unittest.TestCase):
    ...
    def test_fail_closed_cluster_validator(self):
        result = self._run("scripts/validate_6s_cluster.py", "--warnings-as-errors")
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("0 errors, 0 warnings", result.stdout)

    def test_fail_closed_distinctiveness_audit(self):
        result = self._run("scripts/audit_6s_distinctiveness.py")
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("21 component signatures, 0 errors, 0 warnings", result.stdout)

    def test_6n_and_6s_share_one_physical_stylesheet_and_namespace(self):
        self.assertTrue((FUTURES / "currency-research-library.css").is_file())
        self.assertFalse((FUTURES / "6s-research-library.css").exists())
        for filename in (*SIX_N_CLUSTER, *SIX_S_CLUSTER):
            with self.subTest(filename=filename):
                ...
                self.assertNotRegex(html, r'class=["\'][^"\']*\bchf-')
                self.assertNotIn("6s-research-library.css", html)

...
class SixCClusterReleaseTests(unittest.TestCase):
    ...
    def test_fail_closed_cluster_validator(self):
        result = self._run("scripts/validate_6c_cluster.py", "--warnings-as-errors")

        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("0 errors, 0 warnings", result.stdout)

    def test_fail_closed_distinctiveness_audit(self):
        result = self._run("scripts/audit_6c_distinctiveness.py")

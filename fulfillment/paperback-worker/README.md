...
Migration `0006_digital_price_1499.sql` admits the current 1499-cent individual and 4497-cent trilogy totals while preserving the historical 2900- and 6900-cent records and rebuilding the delivery table so its foreign key remains attached to the purchase table. Checkout remains fail-closed unless both bundle release gates are true, the exact live Payment Link ID and Stripe URL are configured, Resend and private R2 are ready, and `/digital-config?bookSlug=market-structure-trilogy` returns the exact 4497-cent offer.

#!/usr/bin/env python3
"""Fail-closed technical and editorial-contract checks for the core 6N library."""
...
MODIFIED_DATES = {
    "6n-common-mistakes.html": "2026-08-16",
    "6n-contract-specs-explained.html": "2026-08-16",
    "6n-correlations.html": "2026-08-16",
    "6n-interest-rate-impact.html": "2026-08-16",
    "6n-liquidity-guide.html": "2026-08-17",
    "6n-multi-timeframe-framework.html": "2026-08-17",
    "6n-risk-sentiment-impact.html": "2026-08-17",
    "6n-seasonal-patterns.html": "2026-08-17",
    "6n-spread-trading.html": "2026-08-18",
    "6n-trading-strategies.html": "2026-08-18",
    "6n-volatility-patterns.html": "2026-08-18",
    "6n-vs-6a-differences.html": "2026-08-18",
    ...
    "why-traders-use-6n-futures.html": "2026-08-20",
}
VISIBLE_MODIFIED_DATES = {
    "2026-08-16": "Updated August 16, 2026",
    "2026-08-17": "Updated August 17, 2026",
...
MOJIBAKE_MARKERS = ("Ã¢â‚¬", "Ã¢â‚¬â„¢", "Ã¢â‚¬â€œ", "Ã¢â‚¬â€", "Ã‚", "Ãƒ", "Ã°Å¸")
GENERIC_H2 = {
...
def normalize_heading(value: str) -> str:
    ...
    value = re.sub(r"\b6n\b|\bnew zealand dollar\b|\bfutures?\b", " ", value)
...
def validate_page(path: Path, id_cache: dict[Path, set[str]]):
    ...
    if "/futures-basics/currency-research-library.css?v=20260820a" not in stylesheets:
        errors.append("missing versioned 6N research stylesheet")
    ...
    if css_path.exists():
        ...
        if undefined_classes:
            errors.append(f"6N component classes lack shared CSS rules: {undefined_classes}")
    ...
    if path.name != CANONICAL_MECHANICS and CANONICAL_MECHANICS not in raw:
        errors.append("page does not link to the canonical 6N contract-mechanics guide")
    ...
    if path.name == CANONICAL_MECHANICS:
        canonical_checks = {
            "100,000 NZD trading unit": r"100,000\s+(?:New Zealand dollars|NZD)",
            "NZD/USD quote convention": r"(?:NZD/USD|U\.S\. dollars? per New Zealand dollar|USD per NZD)",
            "0.00005 Globex outright increment": r"0\.00005\b",
            "$5 Globex outright tick": r"\$5(?:\.00)?\b",
            "0.00001 ClearPort increment": r"0\.00001\b",
            "$1 ClearPort value": r"\$1(?:\.00)?\b",
            ...
            "current listing cycle": r"(?:(?:six|6).{0,100}(?:quarterly|quarterlies).{0,160}(?:March|Mar)|(?:March|Mar).{0,160}(?:December|Dec).{0,160}(?:six|6).{0,50}quarterly)",
            "no invented Micro contract": r"(?:does not (?:currently )?list|no (?:current )?listed|not (?:a )?(?:currently )?listed|is not (?:currently )?listed).{0,120}(?:Micro (?:New Zealand Dollar|NZD)|M6N)",
            "quote-direction explanation": r"higher (?:6N|NZD/USD).{0,140}(?:stronger (?:New Zealand dollar|NZD)|more (?:U\.S\. )?dollars?)",
        ...
        required_cme_sources = (
            "https://www.cmegroup.com/rulebook/CME/III/250/258/258.pdf",
        ...
        if re.search(
            r"(?:current|standard) (?:CME |Globex )?outright (?:tick|increment) (?:is|=|of)\s*(?:0\.0001\b|\$10(?:\.00)?\b)",
            visible_text,
            flags=re.I,
        ):
            errors.append("canonical mechanics page retains the superseded Globex tick specification")

    if path.name == "us-dollar-impact-on-6n.html":
        if not re.search(r"NZD/USD.{0,180}USD/NZD|USD/NZD.{0,180}NZD/USD", visible_text, flags=re.I):
            errors.append("USD-impact guide does not reconcile reciprocal quote conventions")
        if not re.search(r"(?:circular|tautolog|mechanical inverse|reciprocal)", visible_text, flags=re.I):
            errors.append("USD-impact guide lacks a non-circularity warning")

    if path.name == "m6n-micro-contract-guide.html":
        if not re.search(
            r"(?:does not list|no listed|not (?:a )?listed|is not listed).{0,120}(?:M6N|Micro (?:NZD|New Zealand Dollar))",
            visible_text,
            flags=re.I,
        ):
            errors.append("M6N guide must state that CME does not currently list M6N")
        if "https://www.cmegroup.com/markets/microsuite/fx.html" not in raw:
            errors.append("M6N guide lacks the current CME Micro FX product-table source")
        if re.search(
            r"(?:M6N|Micro (?:NZD|New Zealand Dollar)).{0,100}(?:contract (?:size|unit).{0,20}10,000|\$1(?:\.00)? (?:tick|per tick))",
            visible_text,
            flags=re.I,
        ):
            errors.append("M6N guide invents a current Micro NZD contract specification")

    if re.search(
        r"higher (?:6N|NZD/USD).{0,100}(?:weaker (?:New Zealand dollar|NZD)|fewer (?:U\.S\. )?dollars? per NZD)",
        visible_text,
        flags=re.I,
    ):
        errors.append("page reverses the 6N NZD/USD quote direction")
    if re.search(r"(?:6N\s+USD/NZD|USD/NZD\s+6N)\s+futures", visible_text, flags=re.I):
        errors.append("page misnames the exchange contract as USD/NZD futures")
    if re.search(
        r"6N.{0,80}(?:quoted|quotation).{0,80}(?:New Zealand dollars? per U\.S\. dollar|NZD per USD)",
        visible_text,
        flags=re.I,
    ):
...
def main() -> int:
    ...
    print(f"Checked {len(CLUSTER)} core 6N pages: {error_count} errors, {warning_count} warnings.")

#!/usr/bin/env python3
"""Apply the approved staggered 6Z modification dates to sitemap.xml."""
...
try:
    from .validate_6z_cluster import BASE_URL, MODIFIED_DATES, ROOT
except ImportError:  # Direct execution: ``py scripts\\sync_6z_sitemap_dates.py``.
    from validate_6z_cluster import BASE_URL, MODIFIED_DATES, ROOT
...
def main() -> int:
    ...
    print(f"Synchronized {len(MODIFIED_DATES)} core 6Z sitemap dates ({updated} changed).")
    return 0

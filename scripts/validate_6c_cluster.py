#!/usr/bin/env python3
"""Fail-closed technical and editorial-contract checks for the core 6C library."""
...
CLUSTER = (
    "6c-bank-of-canada-impact.html",
    "6c-best-trading-sessions.html",
    "6c-breakout-behavior.html",
    "6c-correlation-risk-on-risk-off.html",
    "6c-fundamental-analysis-guide.html",
    "6c-margin-and-position-sizing.html",
    "6c-oil-correlation.html",
    "6c-orderflow-behavior.html",
    "6c-scalping-guide.html",
    "6c-support-resistance-levels.html",
    "6c-swing-trading-strategy.html",
    "6c-technical-indicators-that-work.html",
    "6c-tick-size-tick-value.html",
    "6c-us-economic-data-impact.html",
    "6c-volatility-cycles.html",
    "6c-volatility-patterns.html",
    "6c-vs-6e-vs-6j.html",
    "best-time-to-trade-6c.html",
    "what-is-6c-futures.html",
    "why-6c-moves.html",
)
CANONICAL_MECHANICS = "6c-tick-size-tick-value.html"
...
def normalize_heading(value: str) -> str:
    ...
    value = re.sub(r"\b6c\b|\bcanadian dollar\b|\bfutures?\b", " ", value)
...
class PageParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs) -> None:
        ...
        if "ca-faq" in set(attr.get("class", "").split()):
            self.faq_depth += 1
...
def visible_faq_entries(raw: str) -> list[tuple[str, str]]:
    ...
    sections = re.findall(
        r'<section\b[^>]*class=["\'][^"\']*\bca-faq\b[^"\']*["\'][^>]*>(.*?)</section>',
        raw,
        flags=re.I | re.S,
    )
...
def source_disclosure(raw: str) -> tuple[str, list[str]]:
    match = re.search(
        r'<(?P<tag>details|section|aside)\b[^>]*class=["\'][^"\']*\bca-sources\b[^"\']*["\'][^>]*>'
        r'(?P<body>.*?)</(?P=tag)>',
        raw,
        flags=re.I | re.S,
    )
...
def validate_page(path: Path, id_cache: dict[Path, set[str]]):
    ...
    if "/futures-basics/6c-research-library.css?v=20260813a" not in stylesheets:
        errors.append("missing versioned 6C research stylesheet")
    if not any("cad-library" in classes for classes in page.main_classes):
        errors.append("main wrapper is missing cad-library class")
    css_path = ARTICLE_DIR / "6c-research-library.css"
    if css_path.exists():
        css_text = css_path.read_text(encoding="utf-8", errors="strict")
        defined_ca_classes = set(re.findall(r"\.(ca-[a-z0-9_-]+)", css_text, flags=re.I))
        used_ca_classes: set[str] = set()
        for class_value in re.findall(r'\bclass=["\']([^"\']+)["\']', raw, flags=re.I):
            used_ca_classes.update(
                value for value in class_value.split() if value.lower().startswith("ca-")
            )
        undefined_classes = sorted(used_ca_classes - defined_ca_classes)
        if undefined_classes:
            errors.append(f"6C component classes lack shared CSS rules: {undefined_classes}")
    ...
    if not source_text:
        errors.append("missing visible ca-sources disclosure")
    else:
    ...
    if not re.search(r'class=["\'][^"\']*ca-disclaimer\b', raw, flags=re.I):
        errors.append("missing visible ca-disclaimer")
    ...
    if path.name != CANONICAL_MECHANICS and CANONICAL_MECHANICS not in raw:
        errors.append("page does not link to the canonical 6C contract-mechanics guide")
    if path.name in EMPIRICAL_PROTOCOL_PAGES and not re.search(
        r"\b(?:no original (?:study|backtest|result|finding)|does not report (?:an? )?original|protocol[,;:]? not (?:a )?result|study has not been run)\b",
        visible_text,
        flags=re.I,
    ):
    ...
    if path.name == CANONICAL_MECHANICS:
        canonical_checks = {
            "standard 100,000 CAD unit": r"100,000\s+(?:Canadian dollars|CAD)",
            "standard 0.00005 increment": r"0\.00005",
            "standard $5 tick": r"\$5(?:\.00)?\b",
            "Micro 10,000 CAD unit": r"10,000\s+(?:Canadian dollars|CAD)",
            "Micro 0.0001 increment": r"0\.0001\b",
            "Micro $1 tick": r"\$1(?:\.00)?\b",
            "physical delivery": r"physically (?:settled|delivered)|physical delivery",
            "termination rule": r"business day immediately preceding the third Wednesday",
            "current 16-month serial listing": r"(?:serial contracts listed for 16 months|16 serial months)",
            "current 20-quarter listing": r"20 consecutive (?:March-cycle )?quarter(?:ly|lies|s)",
            "listing expansion effective date": r"May 10, 2026",
        }
        ...
        if "https://www.cmegroup.com/notices/electronic-trading/2026/04/20260420.html" not in raw:
            errors.append("canonical mechanics page lacks the current CME listing-cycle notice")
        if "CME Globex Notice: April 20, 2026" not in visible_text:
            errors.append("canonical mechanics page mislabels the current CME listing-cycle notice")
        if re.search(r"SER-9515", visible_text, flags=re.I):
            errors.append("canonical mechanics page cites unrelated CME report SER-9515")
        if re.search(r"three nearest serial months", visible_text, flags=re.I):
            errors.append("canonical mechanics page retains the superseded three-serial-month cycle")

    if path.name == "6c-margin-and-position-sizing.html":
        formula_checks = {
            "per-contract denominator formula": (
                r"floor\[loss budget\s*(?:÷|&divide;|/)\s*\(stop loss per contract\s*\+\s*"
                r"fill stress per contract\s*\+\s*round-turn costs per contract\)\]"
            ),
            "$93 stressed loss reconciliation": r"\$75\s*\+\s*\$10\s*\+\s*\$8\s*\$93 per contract",
            "three-contract result": r"3 contracts",
            "$279 reconciled loss": r"\$279",
        }
        for label, pattern in formula_checks.items():
            if not re.search(pattern, visible_text, flags=re.I):
                errors.append(f"position-sizing regression lacks {label}")
        if re.search(r"budget\s*-\s*costs|loss budget\s*(?:−|-)\s*estimated total costs", visible_text, flags=re.I):
            errors.append("position-sizing formula still subtracts per-contract costs in the numerator")

    if path.name == "6c-breakout-behavior.html":
        target_net = 60 - 10 - 10 - 8
        stressed_loss = 40 + 10 + 10 + 8
        if target_net != 32 or stressed_loss != 68:
            errors.append("internal breakout arithmetic regression constants are inconsistent")
        if not re.search(r"stressed loss with entry friction.*?\$68", visible_text, flags=re.I):
            errors.append("breakout cost gate lacks the symmetric $68 stressed-loss result")
        if re.search(r"stressed loss.*?\$58", visible_text, flags=re.I):
            errors.append("breakout cost gate retains the asymmetric $58 stop result")

    if path.name == "6c-support-resistance-levels.html":
        retreat_ticks = round((0.72600 - 0.72555) / 0.00005)
        if retreat_ticks != 9:
            errors.append("internal support/resistance arithmetic regression constant is inconsistent")
        if not re.search(r"nine ticks below anchor", visible_text, flags=re.I):
            errors.append("support/resistance example must reconcile the retreat as nine ticks")
        if re.search(r"seven ticks below anchor", visible_text, flags=re.I):
            errors.append("support/resistance example retains the incorrect seven-tick anchor distance")
...
def main() -> int:
    ...
    css = ARTICLE_DIR / "6c-research-library.css"
    if not css.exists() or css.stat().st_size < 5000:
        all_errors["<shared>"].append("6c-research-library.css is missing or unexpectedly small")
    else:
        css_text = css.read_text(encoding="utf-8", errors="strict")
        event_value_rule = re.search(
            r"\.ca-event-metrics\s+strong\s*\{(?P<body>.*?)\}",
            css_text,
            flags=re.I | re.S,
        )
        if not event_value_rule or not re.search(
            r"\bcolor\s*:\s*var\(--ca-ink\)\s*;",
            event_value_rule.group("body") if event_value_rule else "",
            flags=re.I,
        ):
            all_errors["<shared>"].append(
                "ca-event-metrics strong must set var(--ca-ink) to prevent low-contrast hero inheritance"
            )
    ...
    print(f"Checked {len(CLUSTER)} core 6C pages: {error_count} errors, {warning_count} warnings.")

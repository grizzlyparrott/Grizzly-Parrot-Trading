#!/usr/bin/env python3
"""Fail-closed technical and editorial-contract checks for the core 6Z library."""
...
MODIFIED_DATES = {
    "6z-algorithmic-behavior.html": "2026-08-21",
    "6z-liquidity-map.html": "2026-08-21",
    "6z-margin-requirements.html": "2026-08-21",
    "6z-position-sizing.html": "2026-08-21",
    "6z-seasonal-patterns.html": "2026-08-22",
    "6z-tick-size-and-value.html": "2026-08-22",
    "6z-trade-management-guide.html": "2026-08-22",
    "6z-trading-psychology.html": "2026-08-22",
    "6z-volatility-profile.html": "2026-08-23",
    "6z-vs-6e-vs-6j-differences.html": "2026-08-23",
    "best-indicators-for-6z.html": "2026-08-23",
    "best-times-to-trade-6z-futures.html": "2026-08-23",
    "common-6z-trading-mistakes.html": "2026-08-24",
    "fundamental-drivers-of-6z.html": "2026-08-24",
    "how-sarb-influences-6z.html": "2026-08-24",
    "how-us-dollar-moves-6z.html": "2026-08-24",
    "sarb-rates-impact-6z.html": "2026-08-25",
    "what-are-6z-futures.html": "2026-08-25",
    "why-6z-slippage-hits-harder.html": "2026-08-25",
    "why-6z-trades-differently.html": "2026-08-25",
}
VISIBLE_MODIFIED_DATES = {
    "2026-08-21": "Updated August 21, 2026",
    "2026-08-22": "Updated August 22, 2026",
    "2026-08-23": "Updated August 23, 2026",
    "2026-08-24": "Updated August 24, 2026",
    "2026-08-25": "Updated August 25, 2026",
}
CANONICAL_MECHANICS = "6z-tick-size-and-value.html"
...
def normalize_heading(value: str) -> str:
    ...
    value = re.sub(r"\b6z\b|\bsouth african rand\b|\brand\b|\bzar\b|\bfutures?\b", " ", value)
...
def source_disclosure(raw: str) -> tuple[str, list[str]]:
    ...
    hrefs = re.findall(r'<a\b[^>]*href=["\']([^"\']+)["\']', body, flags=re.I)
    return clean_text(re.sub(r"<[^>]+>", " ", body)), hrefs
...
def validate_cluster_sync(
    ...
    for filename in CLUSTER:
        ...
        elif lastmods[0][:10] != MODIFIED_DATES[filename]:
            errors.append(
                f"sitemap lastmod must be {MODIFIED_DATES[filename]} for {filename}: {lastmods[0]}"
            )
...
def validate_page(path: Path, id_cache: dict[Path, set[str]]):
    ...
    expected_modified = MODIFIED_DATES[path.name]
    expected_visible_modified = VISIBLE_MODIFIED_DATES[expected_modified]
    ...
    if len(re.findall(r"<!--\s*ARTICLE BODY START\s*-->", raw, flags=re.I)) != 1 or len(
        re.findall(r"<!--\s*ARTICLE BODY END\s*-->", raw, flags=re.I)
    ) != 1:
        errors.append("article body markers must each appear exactly once")
    if not re.search(r'<header\b[^>]*class=["\'][^"\']*\bsite-header\b', raw, flags=re.I):
    ...
    if not re.search(
        ...
        errors.append("missing keyboard-visible skip link to #main-content")
    if not re.search(r'<main\b[^>]*\bid=["\']main-content["\']', raw, flags=re.I):
        errors.append("main wrapper must expose id=main-content")
    if not re.search(r'<footer\b[^>]*class=["\'][^"\']*\bsite-footer\b', raw, flags=re.I):
        errors.append("missing shared site footer")
    if not re.search(r'<script\b[^>]*src=["\']/uet\.js["\'][^>]*\bdefer\b', raw, flags=re.I):
        errors.append("missing deferred shared UET script")
    ...
    if meta_value(page, "property", "article:modified_time") != [expected_modified]:
        errors.append(f"article:modified_time must be {expected_modified}")
    ...
    if "/futures-basics/currency-research-library.css?v=20260820a" not in stylesheets:
        errors.append("missing versioned 6Z research stylesheet")
    ...
    if css_path.exists():
        ...
        if undefined_classes:
            errors.append(f"6Z component classes lack shared CSS rules: {undefined_classes}")

    if len(h1s) != 1:
        errors.append(f"must contain exactly one h1, found {len(h1s)}")
    elif title and h1s[0] != title:
        errors.append("visible h1 must equal the HTML title")
    previous = 0
    ...
    if len(articles) != 1:
        ...
        if article.get("dateModified") != expected_modified:
            errors.append(f"Article dateModified must be {expected_modified}")
        if article.get("url") != expected_canonical:
            errors.append("Article url must equal canonical")
        publisher = article.get("publisher", {})
        if (
            not isinstance(publisher, dict)
            or publisher.get("@type") != "Organization"
            or publisher.get("name") != "Grizzly Parrot Trading"
        ):
            errors.append("Article publisher must be the Grizzly Parrot Trading organization")
    ...
    if len(breadcrumbs) != 1:
        ...
        if not isinstance(items, list) or len(items) != 3:
            ...
            if [item.get("position") for item in items] != [1, 2, 3]:
                errors.append("BreadcrumbList positions must be 1, 2, 3")
            if items[0].get("item") != f"{BASE_URL}/":
                errors.append("first breadcrumb item must be the homepage")
            if items[1].get("item") != f"{BASE_URL}/futures-basics/":
                errors.append("second breadcrumb item must be Futures Basics")
            if items[-1].get("item") != expected_canonical:
                errors.append("final breadcrumb item must equal canonical")
            if h1s and clean_text(str(items[-1].get("name", ""))) != h1s[0]:
                errors.append("final breadcrumb name must equal the visible h1")

    published_meta = meta_value(page, "property", "article:published_time")
    if published_meta != [PUBLISHED_DATE]:
        errors.append(f"article:published_time must preserve {PUBLISHED_DATE}")
    if articles and articles[0].get("datePublished") != PUBLISHED_DATE:
        errors.append(f"Article datePublished must preserve {PUBLISHED_DATE}")
    og_titles = meta_value(page, "property", "og:title")
    twitter_titles = meta_value(page, "name", "twitter:title")
    if og_titles != [title] or twitter_titles != [title]:
        errors.append("Open Graph and Twitter titles must equal the HTML title")
    og_descriptions = meta_value(page, "property", "og:description")
    twitter_descriptions = meta_value(page, "name", "twitter:description")
    if description and (og_descriptions != description or twitter_descriptions != description):
        errors.append("Open Graph and Twitter descriptions must equal the meta description")
    ...
    word_count = len(re.findall(r"\b[\wâ€™'-]+\b", article_text(raw), flags=re.UNICODE))
    if word_count < 1100:
    ...
    if expected_visible_modified not in article_text(raw):
        errors.append(f"visible article body lacks {expected_visible_modified}")
    source_text, source_hrefs = source_disclosure(raw)
    if not source_text:
        errors.append("missing visible fx-sources disclosure")
    else:
        if "reviewed august 25, 2026" not in source_text.lower():
            errors.append("source disclosure lacks an August 25, 2026 review statement")
        external_source_hrefs = [
            href for href in source_hrefs if urlparse(href).scheme in {"http", "https"}
        ]
        if len(external_source_hrefs) < 2:
            errors.append("source disclosure must contain at least two external evidence links")
        if any(urlparse(href).scheme != "https" for href in external_source_hrefs):
            errors.append("source disclosure contains a non-HTTPS evidence link")
        if not any(
            any(
                (urlparse(href).hostname or "").lower() == suffix
                or (urlparse(href).hostname or "").lower().endswith("." + suffix)
    ...
    if re.search(
        r"\b(?:guaranteed?|always|never) (?:wins?|works?|moves?|rallies|falls|holds|predicts?)\b"
        r"|\bhigh[- ]probability\b|\bsetups? that (?:actually )?work\b"
        r"|\beasiest (?:contract|market) to trade\b",
        article_text(raw),
        flags=re.I,
    ):
        errors.append("unsupported deterministic or edge-promising language remains")

    visible_text = article_text(raw)
    if path.name != CANONICAL_MECHANICS and CANONICAL_MECHANICS not in raw:
        errors.append("page does not link to the canonical 6Z contract-mechanics guide")
    if path.name in EMPIRICAL_PROTOCOL_PAGES and not re.search(
        r"\b(?:no original (?:study|backtest|result|finding)s?|does not report (?:an? )?original|"
        r"protocol[,;:]? not (?:a )?result|study has not been run|not (?:a )?(?:finding|forecast)|"
        r"framework is not evidence)\b",
        visible_text,
        flags=re.I,
    ):
        errors.append("empirical page does not clearly disclose that no original result is reported")
    if path.name == CANONICAL_MECHANICS:
        canonical_checks = {
            "500,000 ZAR trading unit": r"500,000\s+(?:South African rand|ZAR)",
            "ZAR/USD quote convention": r"(?:ZAR/USD|U\.S\. dollars? per (?:South African )?rand|USD per ZAR)",
            "0.000025 Globex outright increment": r"0\.000025\b",
            "$12.50 Globex outright tick": r"\$12\.50\b",
            "0.000001 ClearPort increment": r"0\.000001\b",
            "$0.50 ClearPort value": r"\$0\.50\b",
            "physical delivery": r"physically (?:settled|delivered)|physical delivery",
            "termination rule": r"second (?:exchange )?business day immediately preceding the third Wednesday",
            "third-Wednesday delivery rule": r"delivery.{0,100}third Wednesday|third Wednesday.{0,100}delivery",
            "no current Micro 6Z counterpart": r"(?:no|does not).{0,120}(?:Micro 6Z|micro South African rand|micro-sized 6Z)",
            "quote-direction explanation": r"higher (?:6Z|ZAR/USD).{0,160}(?:stronger (?:South African )?rand|more (?:U\.S\. )?dollars?)",
            "current regular-hours context": r"5:00 p\.m\..{0,80}4:00 p\.m\..{0,30}(?:CT|Chicago time)",
        }
        for label, pattern in canonical_checks.items():
            if not re.search(pattern, visible_text, flags=re.I):
                errors.append(f"canonical mechanics page lacks {label}")
        required_cme_sources = (
            "https://www.cmegroup.com/rulebook/CME/III/250/259/259.pdf",
            "https://www.cmegroup.com/markets/fx/fx-product-guide.html",
            "https://www.cmegroup.com/notices/clearport/2023/08/20230831.html",
        )
        for source in required_cme_sources:
            if source not in raw:
                errors.append(f"canonical mechanics page lacks authoritative CME source {source}")
        if re.search(
            r"(?:current|standard) (?:CME |Globex )?outright (?:tick|increment) (?:is|=|of)\s*(?:0\.00001\b|0\.000001\b|\$5\.00\b|\$0\.50\b)",
            visible_text,
            flags=re.I,
        ):
            errors.append("canonical mechanics page confuses the smaller ClearPort increment with the Globex outright tick")

    if path.name == "what-are-6z-futures.html":
        if not re.search(r"ZAR/USD.{0,180}USD/ZAR|USD/ZAR.{0,180}ZAR/USD", visible_text, flags=re.I):
            errors.append("orientation guide does not reconcile reciprocal quote conventions")
        if not re.search(r"(?:reciprocal|invert|1\s*/)", visible_text, flags=re.I):
            errors.append("orientation guide lacks a reciprocal-quote warning")

    if re.search(
        r"higher (?:6Z|ZAR/USD).{0,120}(?:weaker (?:South African )?rand|fewer (?:U\.S\. )?dollars? per ZAR)",
        visible_text,
        flags=re.I,
    ):
        errors.append("page reverses the 6Z ZAR/USD quote direction")
    if re.search(r"(?:6Z\s+USD/ZAR|USD/ZAR\s+6Z)\s+futures", visible_text, flags=re.I):
        errors.append("page misnames the exchange contract as USD/ZAR futures")
    if re.search(
        r"6Z.{0,80}(?:quoted|quotation).{0,80}(?:South African rand per U\.S\. dollar|ZAR per USD)",
        visible_text,
        flags=re.I,
    ):
        errors.append("page reverses the exchange quotation convention")

    table_count = len(re.findall(r"<table\b", raw, flags=re.I))
    region_table_count = len(
        re.findall(
            r'<(?:div|section)\b(?=[^>]*\brole=["\']region["\'])'
            r'(?=[^>]*\baria-label=["\'][^"\']+["\'])'
            r'(?=[^>]*\btabindex=["\']0["\'])[^>]*>.*?<table\b',
            raw,
            flags=re.I | re.S,
        )
    )
    ...
    for index, table in enumerate(
        re.findall(r"<table\b[^>]*>.*?</table>", raw, flags=re.I | re.S),
        start=1,
    ):
        if not re.search(r"<thead\b", table, flags=re.I):
            errors.append(f"table {index} lacks a semantic thead")
        if not re.search(r"<th\b[^>]*\bscope=[\"\'](?:col|row)[\"\']", table, flags=re.I):
            errors.append(f"table {index} lacks scoped header cells")
...
def main() -> int:
    ...
    print(f"Checked {len(CLUSTER)} core 6Z pages: {error_count} errors, {warning_count} warnings.")

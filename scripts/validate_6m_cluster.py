#!/usr/bin/env python3
"""Fail-closed technical and editorial-contract checks for the core 6M library."""
...
CLUSTER = (
    "6m-backtesting.html",
    "6m-best-indicators.html",
    "6m-best-times.html",
    "6m-carry-trade.html",
    "6m-data-traps.html",
    "6m-fundamental-drivers.html",
    "6m-margin-requirements.html",
    "6m-seasonality.html",
    "6m-slippage.html",
    "6m-tick-size.html",
    "6m-trade-and-remittances.html",
    "6m-trading-mistakes.html",
    "6m-trading-plan.html",
    "6m-volatility-profile.html",
    "6m-vs-6e-vs-6j.html",
    "banxico-influence-6m.html",
    "banxico-rate-policy-6m.html",
    "usd-strength-6m.html",
    "what-are-6m-futures.html",
    "why-6m-trades-differently.html",
)
CANONICAL_MECHANICS = "6m-tick-size.html"
...
def normalize_heading(value: str) -> str:
    ...
    value = re.sub(r"\b6m\b|\bmexican peso\b|\bfutures?\b", " ", value)
...
def validate_page(path: Path, id_cache: dict[Path, set[str]]):
    ...
    if css_path.exists():
        ...
        if undefined_classes:
            errors.append(f"6M component classes lack shared CSS rules: {undefined_classes}")
    ...
    if path.name != CANONICAL_MECHANICS and CANONICAL_MECHANICS not in raw:
        errors.append("page does not link to the canonical 6M contract-mechanics guide")
    ...
    if path.name == CANONICAL_MECHANICS:
        canonical_checks = {
            "500,000 MXN trading unit": r"500,000\s+(?:Mexican pesos|MXN)",
            "MXN/USD quote convention": r"(?:MXN/USD|U\.S\. dollars? per Mexican peso|USD per MXN)",
            "0.00001 Globex outright increment": r"0\.00001\b",
            ...
            "current listing cycle": r"13 consecutive (?:calendar )?months.{0,120}(?:two|2) (?:additional |deferred )?(?:March(?:-cycle)? )?quarter",
            "no invented Micro contract": r"(?:does not list|no listed|not a listed).{0,80}Micro (?:Mexican Peso|MXN|6M)",
            "quote-direction explanation": r"higher (?:6M|MXN/USD).{0,100}(?:stronger (?:Mexican )?peso|stronger MXN)",
        ...
        required_cme_sources = (
            "https://www.cmegroup.com/rulebook/CME/III/250/256/256.pdf",
            "https://www.cmegroup.com/articles/faqs/latam-fx-futures-frequently-asked-questions.html",
        )
        for source in required_cme_sources:
            if source not in raw:
                errors.append(f"canonical mechanics page lacks authoritative CME source {source}")
        if re.search(r"(?:current|standard).{0,40}(?:0\.000025|\$12\.50)", visible_text, flags=re.I):
            errors.append("canonical mechanics page retains the superseded Globex tick specification")

    if path.name == "6m-margin-requirements.html":
        margin_checks = {
            "margin is not maximum loss": r"margin.{0,120}(?:is not|does not).{0,100}(?:maximum|max|cap).{0,40}loss",
            "integer loss-budget sizing": r"(?:floor|round down).{0,120}(?:loss budget|risk budget).{0,160}(?:stop|invalidation)",
            "gap or slippage stress": r"(?:gap|slippage|fill stress)",
            "variable exchange or broker requirement": r"(?:exchange|broker).{0,100}(?:change|vary|raise|higher)",
        }
        for label, pattern in margin_checks.items():
            if not re.search(pattern, visible_text, flags=re.I):
                errors.append(f"margin guide lacks {label}")

    if path.name == "usd-strength-6m.html":
        if not re.search(r"MXN/USD.{0,180}USD/MXN|USD/MXN.{0,180}MXN/USD", visible_text, flags=re.I):
            errors.append("USD-strength guide does not reconcile reciprocal quote conventions")
        if not re.search(r"(?:circular|tautolog|mechanical inverse|reciprocal)", visible_text, flags=re.I):
            errors.append("USD-strength guide lacks a non-circularity warning")

    if re.search(
        r"higher (?:6M|MXN/USD).{0,80}(?:weaker (?:Mexican )?peso|weaker MXN)",
        visible_text,
        flags=re.I,
    ):
        errors.append("page reverses the 6M MXN/USD quote direction")
    if re.search(r"(?:6M\s+USD/MXN|USD/MXN\s+6M)\s+futures", visible_text, flags=re.I):
        errors.append("page misnames the exchange contract as USD/MXN futures")
    if re.search(
        r"6M.{0,80}(?:quoted|quotation).{0,80}(?:Mexican pesos? per U\.S\. dollar|MXN per USD)",
        visible_text,
        flags=re.I,
    ):
...
def main() -> int:
    ...
    print(f"Checked {len(CLUSTER)} core 6M pages: {error_count} errors, {warning_count} warnings.")

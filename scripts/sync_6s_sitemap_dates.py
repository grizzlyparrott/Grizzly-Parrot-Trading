#!/usr/bin/env python3
"""Apply the approved staggered 6S modification dates to sitemap.xml."""
...
try:
    from .validate_6s_cluster import BASE_URL, MODIFIED_DATES, ROOT
except ImportError:  # Direct execution: ``py scripts\\sync_6s_sitemap_dates.py``.
    from validate_6s_cluster import BASE_URL, MODIFIED_DATES, ROOT
...
def main() -> int:
    ...
    print(f"Synchronized {len(MODIFIED_DATES)} core 6S sitemap dates ({updated} changed).")
    return 0

#!/usr/bin/env python3
"""Fail-closed technical and editorial-contract checks for the core 6S library."""
...
MODIFIED_DATES = {
    ...
    "6s-how-us-economic-data-moves-swiss-franc-futures.html": "2026-08-18",
    "6s-impact-of-global-risk-events-how-chf-reacts-to-shocks.html": "2026-08-18",
    "6s-intraday-yield-tracking-how-bond-moves-guide-chf-futures.html": "2026-08-19",
    "6s-london-fix-liquidity-shifts-and-daily-flows.html": "2026-08-19",
    "6s-macro-triggers-ranking-the-events-that-move-chf-the-most.html": "2026-08-19",
    "6s-mean-reversion-setups-and-why-they-work.html": "2026-08-19",
    "6s-safe-haven-flows-and-why-chf-surges-in-market-panics.html": "2026-08-20",
    "6s-safe-haven-vs-jpy-which-leads-in-risk-off.html": "2026-08-20",
    "6s-session-overlaps-why-europe-us-handover-moves-chf-futures.html": "2026-08-20",
    "6s-top-correlations-for-swiss-franc-futures.html": "2026-08-20",
    "6s-typical-liquidity-behavior-sweeps-fakeouts-slow-drifts.html": "2026-08-21",
    "6s-volatility-compression-and-breakout-behavior.html": "2026-08-21",
    "6s-what-moves-swiss-franc-futures.html": "2026-08-21",
    "6s-why-6s-has-low-volatility-and-how-to-trade-it.html": "2026-08-21",
    "6s-yield-spreads-and-how-interest-rate-differentials-drive-price.html": "2026-08-21",
}
...
def normalize_heading(value: str) -> str:
    ...
    value = re.sub(r"\b6s\b|\bswiss franc\b|\bfutures?\b", " ", value)
...
def validate_page(path: Path, id_cache: dict[Path, set[str]]):
    ...
    if "/futures-basics/currency-research-library.css?v=20260820a" not in stylesheets:
        errors.append("missing versioned 6S research stylesheet")
    ...
    if css_path.exists():
        ...
        if undefined_classes:
            errors.append(f"6S component classes lack shared CSS rules: {undefined_classes}")
    ...
    if not source_text:
        ...
        if "reviewed august 21, 2026" not in source_text.lower():
            errors.append("source disclosure lacks an August 21, 2026 review statement")
    ...
    if path.name != CANONICAL_MECHANICS and CANONICAL_MECHANICS not in raw:
        errors.append("page does not link to the canonical 6S contract-mechanics guide")
    ...
    if path.name == CANONICAL_MECHANICS:
        canonical_checks = {
            "125,000 CHF trading unit": r"125,000\s+(?:Swiss francs|CHF)",
            "CHF/USD quote convention": r"(?:CHF/USD|U\.S\. dollars? per Swiss franc|USD per CHF)",
            "0.00005 Globex outright increment": r"0\.00005\b",
            "$6.25 Globex outright tick": r"\$6\.25\b",
            "0.00001 ClearPort increment": r"0\.00001\b",
            "$1.25 ClearPort value": r"\$1\.25\b",
            ...
            "March quarterly cycle": r"(?:March|Mar).{0,100}(?:June|Jun).{0,100}(?:September|Sep).{0,100}(?:December|Dec)",
            "current Micro Swiss Franc distinction": r"(?:MSF|Micro Swiss Franc).{0,160}12,500\s+(?:Swiss francs|CHF)",
            "quote-direction explanation": r"higher (?:6S|CHF/USD).{0,140}(?:stronger (?:Swiss franc|CHF)|more (?:U\.S\. )?dollars?)",
        ...
        required_cme_sources = (
            "https://www.cmegroup.com/rulebook/CME/III/250/254/254.pdf",
        ...
        if re.search(
            r"(?:current|standard) (?:CME |Globex )?outright (?:tick|increment) (?:is|=|of)\s*(?:0\.0001\b|\$12\.50\b)",
            visible_text,
            flags=re.I,
        ):
            errors.append("canonical mechanics page retains the superseded Globex tick specification")

    if path.name == "6s-chf-usd-spot-vs-futures-differences.html":
        if not re.search(r"CHF/USD.{0,180}USD/CHF|USD/CHF.{0,180}CHF/USD", visible_text, flags=re.I):
            errors.append("spot-versus-futures guide does not reconcile reciprocal quote conventions")
        if not re.search(r"(?:circular|tautolog|mechanical inverse|reciprocal)", visible_text, flags=re.I):
            errors.append("spot-versus-futures guide lacks a non-circularity warning")

    if re.search(
        r"higher (?:6S|CHF/USD).{0,100}(?:weaker (?:Swiss franc|CHF)|fewer (?:U\.S\. )?dollars? per CHF)",
        visible_text,
        flags=re.I,
    ):
        errors.append("page reverses the 6S CHF/USD quote direction")
    if re.search(r"(?:6S\s+USD/CHF|USD/CHF\s+6S)\s+futures", visible_text, flags=re.I):
        errors.append("page misnames the exchange contract as USD/CHF futures")
    if re.search(
        r"6S.{0,80}(?:quoted|quotation).{0,80}(?:Swiss francs? per U\.S\. dollar|CHF per USD)",
        visible_text,
        flags=re.I,
    ):
...
def main() -> int:
    ...
    print(f"Checked {len(CLUSTER)} core 6S pages: {error_count} errors, {warning_count} warnings.")

#!/usr/bin/env python3
"""Synchronize the two existing guide hubs with the rebuilt core 6C pages only."""
...
try:
    from .validate_6c_cluster import ARTICLE_DIR, BASE_URL, CLUSTER, ROOT
except ImportError:  # Direct execution: ``py scripts\\sync_6c_hubs.py``.
    from validate_6c_cluster import ARTICLE_DIR, BASE_URL, CLUSTER, ROOT
...
def sync_currency_hub(metadata: dict[str, tuple[str, str]]) -> None:
    ...
    if not cluster_cards:
        raise ValueError("Currency hub contains no existing core 6C insertion anchor")
...
def sync_futures_hub(metadata: dict[str, tuple[str, str]]) -> None:
    ...
    if missing:
        raise ValueError(f"Futures hub is missing core 6C cards: {sorted(missing)}")
    path.write_text(updated, encoding="utf-8", newline="")
...
def main() -> int:
    ...
    print(f"Synchronized {len(CLUSTER)} core 6C pages across both hubs.")
    return 0

#!/usr/bin/env python3
"""Synchronize the two existing guide hubs with the rebuilt core 6B pages only."""
...
from validate_6b_cluster import ARTICLE_DIR, BASE_URL, CLUSTER, ROOT
...
def read_metadata(filename: str) -> tuple[str, str]:
    ...
    if not title_match or not desc_match:
        raise ValueError(f"{filename} is missing title or meta description")
...
def sync_currency_hub(metadata: dict[str, tuple[str, str]]) -> None:
    ...
    if not cluster_cards:
        raise ValueError("Currency hub contains no existing core 6B insertion anchor")
...
def sync_futures_hub(metadata: dict[str, tuple[str, str]]) -> None:
    ...
    if missing:
        raise ValueError(f"Futures hub is missing core 6B cards: {sorted(missing)}")
    path.write_text(updated, encoding="utf-8", newline="")
...
def main() -> int:
    ...
    print(f"Synchronized {len(CLUSTER)} core 6B pages across both hubs.")
    return 0
